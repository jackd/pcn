...
def ragged_down_sample_query(coords,
    ...
    for t in (sample_indices, query_indices, query_counts):
        t.set_shape((None,))
    query_indices = tf.RaggedTensor.from_row_lengths(query_indices,
                                                     query_counts,
                                                     validate=False)
...
def knn_query(in_coords: tf.Tensor, out_coords: tf.Tensor, k: int):
    ...
    dists, indices = tf.numpy_function(fn, (in_coords, out_coords),
                                       (tf.float32, tf.int64))
...
def ragged_ifp_sample_precomputed(dists,
                                  indices,
                                  row_lengths,
                                  sample_size,
                                  eps: float = 1e-8):
    ...
    sample_indices.set_shape(
        (sample_size if isinstance(sample_size, int) else None,))
...
def rect_ifp_sample_precomputed(dists, indices, sample_size, eps: float = 1e-8):
    ...
    return ragged_ifp_sample_precomputed(dists,
                                         indices,
                                         row_lengths,
