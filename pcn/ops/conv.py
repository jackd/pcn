...
from kblocks.tf_typing import TensorOrVariable, DenseShape
...
def featureless_conv(edge_features: tf.Tensor, sparse_indices: tf.Tensor,
                     kernel: TensorOrVariable, out_size: tf.Tensor):
    """
    ...
    with tf.name_scope('featureless_conv'):
        ...
        neigh_sum = tf.math.unsorted_segment_sum(edge_features,
                                                 i,
...
def get_reordered_sparse_transform(sparse_indices: tf.Tensor,
    ...
    def fn(features, edge_features):
        sp = tf.SparseTensor(sparse_indices, tf.gather(edge_features, order),
                             dense_shape)
        return tf.sparse.sparse_dense_matmul(sp, features)
...
def get_gather_sum_transform(sparse_indices: tf.Tensor,
...
def sparse_conv(features: tf.Tensor,
    ...
    return map_reduce_sum(get_term, (kernel, edge_features),
...
def block_conv(features: tf.Tensor,
               kernel: TensorOrVariable,
               sparse_indices: tf.Tensor,
               edge_features: tf.Tensor,
    ...
    if transform_first:
        ...
        sp = tf.sparse.reorder(
            tf.SparseTensor(tf.stack((i, j), axis=-1), edge_features,
                            (N_out, T * N_in)))
        ...
        features = term_impl(tf.stack((i, j), axis=-1),
                             (T * N_out, N_in))(features, edge_features)
...
def sparse_flex_up_sample(features: tf.Tensor, sparse_indices: tf.Tensor):
    ...
    return tf.math.unsorted_segment_max(tf.gather(features, i, axis=0),
                                        j,
                                        num_segments=N_in)
