...
def ip_pool_build(coords,
    ...
    if valid_size is not None:
        mask = tf.expand_dims(tf.sequence_mask(valid_size,
                                               tf.shape(features)[0]),
                              axis=-1)
...
class ComponentsTest(tf.test.TestCase):
    ...
    def test_ip(self):
        for normalize in (False, True):
            self._test_build_fn(functools.partial(ip_build,
                                                  normalize=normalize))
    ...
    def test_ip_pool(self):
        for bucket_size in (False, True):
            for normalize in (False, True):
                self._test_build_fn(
                    functools.partial(ip_pool_build,
                                      normalize=normalize,
    ...
    def test_ip_ds_pool(self):
        for bucket_size in (False, True):
            for normalize in (False, True):
                self._test_build_fn(
                    functools.partial(ip_ds_pool_build,
                                      normalize=normalize,
                                      bucket_size=bucket_size))
