...
def _double_search_radius_kwargs(
    in_place_radius, in_place_k, down_sample_radius, down_sample_k
):
    return dict(
        ...
        down_sample_k=None if down_sample_k is None else down_sample_k * 4,
...
def resnet(
    ...
    for i in range(1, num_levels):
        ...
        out_cloud, ip_neigh, ds_neigh = in_cloud.sample_query(
            **kwargs, edge_features_fn=edge_features_fn, weight_fn=weight_fn,
        )
...
def resnet_segmentation(
    ...
    if object_class is not None:
        object_class = mg.model_input(mg.batch(mg.cache(inputs["object_class"])))
    ...
    for i in range(num_levels - 1, -1, -1):
        ...
        # in place
        features = identity_block(
            features, ip_neighs.pop(), dropout_rate, bottleneck, name=f"us-ip{i}"
        )
